locals {
  name_prefix = "ascending-${var.app_name}"
}
...
resource "aws_cloudfront_distribution" "s3_distribution" {
  ...
  origin {
    domain_name              = data.aws_s3_bucket.cloudfront_s3.bucket_domain_name
    origin_access_control_id = aws_cloudfront_origin_access_control.cloudfront_s3.id
    origin_id                = "${local.name_prefix}-frontend.s3.us-east-1.amazonaws.com"
    origin_path              = "/${var.env}"
  }
  ...
  enabled             = true
  comment             = "jarvis demo frontend distribution by terraform"
  ...
  default_cache_behavior {
    ...
    target_origin_id = "${local.name_prefix}-frontend.s3.us-east-1.amazonaws.com"
